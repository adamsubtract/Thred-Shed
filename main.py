...
# ANSWER: name, amount, color, date

# 2.
#
# It looks like each transaction is separated from the next transaction by a ,,
#
# and then each piece of data within a transaction is separated by the artifact ;,;.
#
# If we want to split up daily_sales into a list of individual transactions, we are going to want to split
#
# by ,, but first, we need to replace the artifact ;,; to something without a comma, so we don’t split any transactions
#
# themselves.
#
...
# 6.Now, iterate through daily_transactions (remember, this is a list of strings currently), and for each transaction,
#
...
#
# Now, Iterate through daily_transactions_split and for each transaction iterate through the different data points and
#
...
# 14. Now, consider the list sales. It is a list of strings that we want to sum.
#
# In order for us to sum these values, we will have to remove the $, and set them equal to floats.
#
...
# 17.
# We see that thread_sold is a list of strings,
#
# some are single colors and some are multiple colors separated by the & character.
#
...
#
# first split the string around the & character and then add each color individually to thread_sold_split.
...
#
# The function should iterate through thread_sold_split and count the number of times the item is equal to argument,
#
# color. Then, it should return this count.
...
# 22.
# Now, using the string method .format() and the function color_count(),
#
# iterate through the list colors and print a sentence that says how many threads of each color were sold today.

for color in colors:
    print("Thread Shed sold {count} of {color} thread today".format(count=color_count(color), color=color))
